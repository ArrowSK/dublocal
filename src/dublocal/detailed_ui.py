...
# Keep the ordinary detailed workflow hardware-aware while preserving the explicit
# legacy translation path. These are the active bindings previously introduced by the
# v0.4.2 overlay.
...
# Stable base operations captured once. Product UI may temporarily replace the public
# detailed helpers, but these remain the underlying engines for the compatibility-safe
# wrappers below.
...
# The base builder wires these callbacks by name. Bind the hardware-aware versions
# directly instead of adding a versioned UI layer around the builder.
...
def _scan_source_ui(*args, **kwargs):
    """Remember the source and curate the caption inventory before it reaches the UI."""

    global _LAST_SOURCE_LANGUAGE, _LAST_AUTO_VOICE_SUMMARY
    result = list(_ORIGINAL_SCAN(*args, **kwargs))
...
def build_app() -> gr.Blocks:
    """Build the current detailed workflow from one compatibility-safe layer."""
    ...
    def accordion_wrapper(*args, **kwargs):
        ...
        recommendation = active_recommendation()
        if label == "Contextual translation · Qwen3 4B":
            if args:
                args = (
    ...
    if m5 and m5_required.issubset(captured):
        with demo:
    ...
    if package and package_required.issubset(captured):
        with demo:
